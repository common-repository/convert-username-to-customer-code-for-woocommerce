...
function enqueue_assets() {
    ...
    wp_enqueue_script( 'change-username', plugins_url( '/js/script.js', CUTCCFWCHANGE_USERNAME_FILE ), array(), CUTCCFWCHANGE_USERNAME_VERSION, true );
...
function ajax_handler() {
    ...
    // check caps
    if( ! current_user_can( 'edit_users' ) ) {
        $response['message'] = 'Non hai i permessi per procedere con questa operazione.';
    ...
    if( empty( $_POST['new_username'] ) || $_POST['old_username'] ) {
        $response['message'] = 'Richiesta Invalida.';
        wp_send_json($response);
    }
    ...
    if( ! validate_username( $new_username ) ) {
        $response['message'] = __( 'Questo codice cliente non è valido perché utilizzi caratteri illegali. Per favore inserisci caratteri validi. ' );
        wp_send_json($response);
    }
    ...
    if ( in_array( $new_username, $illegal_user_logins ) ) {
        $response['message'] =  __( 'Spiacente, non possiamo inserire questa tipologia di codice.' );
        wp_send_json($response);
    }

    // check if new username is in use already
    if( username_exists( $new_username ) ) {
        $response['message'] = sprintf( '<strong>%s</strong> è già utilizzato.', $new_username );
        wp_send_json($response);
    }

    // change the username
    $old_username = sanitize_user( $_POST['current_username'] );
    change_username( $old_username, $new_username );

    // success response
    $response['success'] = true;
    $response['message'] = sprintf( 'Codice cliente cambiato con successo con <strong>%s</strong>.', $new_username );
    wp_send_json($response);
}
